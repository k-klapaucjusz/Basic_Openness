    ...
    public class ApiWrapper : INotifyPropertyChanged
    {
        ...
        { get => _licznik;
            set
            {
                ...
                {
                    _licznik = value;
                    NotifyPropertyChanged(nameof(Licznik));
                }
            }
        }
        //moje testy 
        ...
        public TiaPortalMode TiaPortalMode
        {
            ...
            set
            {
                if (value != _tiaPortalMode)
                {
                    ...
                    NotifyPropertyChanged(nameof(TiaPortalMode));
                }
            }
        }
            ...
            if (_tiaPortalProcessList.Count > 0)
            {
                foreach (var item in _tiaPortalProcessList)
                {
                    if (item.ProjectPath != null && item.ProjectPath.FullName == path)
                    {
                        ...
                        if (System.Windows.MessageBox.Show($"Do you want to connect to the instance (process: {processId})?", "The project is already open", System.Windows.MessageBoxButton.YesNo, System.Windows.MessageBoxImage.Question) == System.Windows.MessageBoxResult.Yes)
                        {
                            accessGranted = true;
                        }
                        break;
